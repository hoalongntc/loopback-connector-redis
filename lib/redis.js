...
exports.initialize = function initializeSchema(dataSource, callback) {
  ...
  if (dataSource.settings.db || dataSource.settings.database) {
    dataSource.settings.options.db = dataSource.settings.db ||
      dataSource.settings.database;
  }
  if (dataSource.settings.password) {
    dataSource.settings.options.password = dataSource.settings.password;
  }
...
function replaceExec(client) {
  client._exec = client.exec;
  client.exec = function(cb) {
    return client._exec.call(function(err, results) {
      if (!results) return cb(err, results);
      cb(err, results.map(function(item) {
        return item.pop();
      }));
    });
  };
...
function Client(client) {
  ...
  client._multi = client.multi;
  client.multi = function(args, cb) {
    if (!cb) {
      var trans = multi.call(client, args);
      replaceExec(trans);
      return trans;
    }
    client._multi.call(client, args).exec(function(err, results) {
      if (!results) return cb(err, results);
      cb(err, results.map(function(item) {
        return item.pop();
      }));
...
Client.prototype.multi = function(commands, callback) {
  ...
  this._client.multi(commands, function(err, replies) {
    if (err) console.log(err);
    log();
    if (callback)
...
BridgeToRedis.prototype.save = function(model, data, callback) {
  var nullFields = Object.keys(data).filter(function(key) {
  ...
  this.client.hgetall(model + ':' + data.id, function(err, prevData) {
    ...
    var schedule = [['HMSET', model + ':' + data.id, data]];
    if (nullFields.length) {
      schedule.push(['HDEL', model + ':' + data.id].concat(nullFields));
    }
